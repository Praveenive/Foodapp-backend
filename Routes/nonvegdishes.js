...
import { Nonvegdish} from "../Models/nonvegdishes.js";
...
router.post("/add-dish",async(req,res)=>{
    try {
        const newDish = await new Nonvegdish({
...
router.get("/alldishes",async(req,res)=>{
    try {
        const alldishes = await Nonvegdish.find()
        if(!alldishes){
...
router.put("/updatedish/:id",async(req,res)=>{
    try {
        const updatedish = await Nonvegdish.findOneAndUpdate(
            {_id:req.params.id},
            {$set:req.body},
            {new:true}
        )
...
router.delete("/delete/:id",async(req,res)=>{
    try {
        const delDish = await Nonvegdish.findByIdAndDelete({_id:req.params.id})
        if(!delDish){
...
export const nonvegdishRouter = router;